...
function prepColors(input?: string[]) {
  const base = (input && input.length ? input : ['#ffffff', '#c8c8c8', '#9e9e9e']).slice(0, MAX_COLORS)
...
/**
 * WebGL-фон ferrofluid из React Bits (ogl).
 * @see https://reactbits.dev/backgrounds/ferrofluid
 */
export function Ferrofluid({
  className,
  dpr,
  paused = false,
  colors = ['#ffffff', '#c8c8c8', '#9e9e9e'],
  speed = 0.5,
  scale = 1.6,
  turbulence = 1,
  fluidity = 0.1,
  ...
  useEffect(() => {
    ...
    /* Слушаем окно: canvas лежит под контентом и сам pointer не ловит */
    if (mouseInteraction) {
  ...
  return (
    <div
      ref={containerRef}
      className={className ? `ferrofluid-container ${className}` : 'ferrofluid-container'}
